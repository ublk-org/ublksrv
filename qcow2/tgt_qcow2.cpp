...
static inline void io_uring_prep_rw_zc(struct io_uring_sqe *sqe,
		struct io_uring_sqe *secondary,
		int op, int dev_fd,
		const struct ublksrv_io_desc *iod, int tag, int q_id, __u8 fd,
		u64 offset)
{
	io_uring_prep_grp_lead(sqe, dev_fd, tag, q_id, 0);
	sqe->user_data = build_user_data(tag, op, 1, 1);
		...
		const struct ublksrv_io_desc *iod)
{
	...
	if (use_zc) {
		...
		io_uring_prep_rw_zc(sqe2, sqe, io_op, 0, iod, tag, q->q_id, 1,
				offset);
	} else {
