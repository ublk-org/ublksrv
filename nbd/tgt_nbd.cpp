...
struct nbd_queue_data {
	...
	struct nbd_sqe_pair last_send_sqes;
		...
		unsigned len, unsigned msg_flags)
{
	...
	io_uring_sqe_set_flags(secondary, IOSQE_FIXED_FILE | IOSQE_IO_LINK);
}
...
{
	struct nbd_sqe_pair *pair = &q_data->last_send_sqes;
		...
		struct nbd_queue_data *q_data)
{
	...
	if (use_send_zc) {
		...
		nbd_set_chain_tail(q_data, NULL, sqe2);
	} else {
		...
		io_uring_prep_send_ublk_zc(sqe3, sqe2, 0, data->tag, q->q_id, fd,
				...
				msg->msg_iov[1].iov_len, msg_flags);
		nbd_set_chain_tail(q_data, sqe3, sqe2);
	}
		...
		struct nbd_queue_data *q_data)
{
	...
	nbd_set_chain_tail(q_data, NULL, sqe);
	return 1;
}
...
static void nbd_mark_chain_tail(struct nbd_queue_data *q_data)
{
	...
	if (pair->sqe) {
		pair->sqe->flags &= ~IOSQE_IO_LINK;
		pair->sqe = NULL;
	}
	if (pair->sqe2) {
		pair->sqe2->flags &= ~IOSQE_IO_LINK;
		pair->sqe2 = NULL;
	}
}
