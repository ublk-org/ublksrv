...
#define UBLK_U_CMD_REG_BPF		\
	_IOR('u', 0x15, struct ublksrv_ctrl_cmd)
#define UBLK_U_CMD_UNREG_BPF	\
	_IOR('u', 0x16, struct ublksrv_ctrl_cmd)

/*
 * UBLK_U_CMD_REG_BPF parameter
 *
 * passed from top byte of cmd->data[0]
 */
enum ublk_bpf_type {
	UBLK_BPF_QUEUE_CMD = 0,
	UBLK_BPF_IO_TASK,
	UBLK_BPF_TYPE_MAX,
};
...
/*
 * Set blk-mq's BLK_MQ_F_BLOCKING, so that bpf prog of UBLK_BPF_QUEUE_CMD
 * can be sleepable
 */
#define UBLK_F_BLOCKING		(1UL << 10)
