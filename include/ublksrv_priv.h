...
struct ublksrv_queue_batch {
	struct batch_fetch_buf fetch_bufs[2];	/* 2 fetch buffers (double-buffering) */
	struct batch_commit_buf commit_bufs[2];	/* 2 commit buffers (one active, one in-flight) */
	...
	unsigned char commit_buf_inflight;	/* Number of commit buffers currently in-flight */
