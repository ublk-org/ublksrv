...
int ublksrv_batch_alloc_bufs(struct _ublksrv_queue *q)
{
	...
	/* Allocate commit buffer memory (2 buffers) */
	ret = posix_memalign(&buf, page_sz, b->commit_buf_size * 2);
	if (ret || !buf)
		return -ENOMEM;
	...
	if (mlock(b->commit_buf_mem, b->commit_buf_size * 2))
	...
	/* Allocate fetch buffers (2 buffers, page-aligned) */
...
fail_fetch:
	...
	munlock(b->commit_buf_mem, b->commit_buf_size * 2);
...
void ublksrv_batch_start_fetch(struct _ublksrv_queue *q)
{
	...
	/* Issue two multishot fetch commands (double-buffering) */
	ublksrv_batch_queue_fetch(q, 0);
	ublksrv_batch_queue_fetch(q, 1);
